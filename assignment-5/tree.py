...
class DecisionTree:
    ...
    def build_tree(self, X, Y, depth):
        ...
        #calculating the propabilities of classes
        for c in self.classes:
                ...
                if ni!=0:
                    prior_prob=float(ni)/nexamples
                else:
                    prior_prob=0
                priorAll.append(prior_prob)

            
        purityD_Idx=np.argmax(priorAll)
        purityD=priorAll[purityD_Idx]
        
        
        node=Node()
        if  purityD>=self.purity or nexamples<=self.exthreshold or depth>self.maxdepth:
            
            node.klasslabel=self.classes[purityD_Idx]
            node.purity=purityD
            if self.pdist:
                node.pdistribution=priorAll
            return node
       
        learner=self.getWeakLearner()
        
        score,Xlidx,Xridx=learner.train(X,Y)
        
        node.wlearner=learner
        node.score=score
        ...
        lft_CN=self.build_tree(DY,DY_lbl,depth+1)
        rgt_CN=self.build_tree(DN,DN_lbl,depth+1)
        
        node.set_childs(lft_CN,rgt_CN)
    ...
    def _predict(self,node, X):
        ...
        #--------Write Your Code Here ---------------------#
        if node.isleaf():
            label=node.klasslabel
            return label
        elif node.isless_than_eq(X):
