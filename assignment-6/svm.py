...
class SVM(Classifier):
    ...
    def cost_function(self, X, Y, theta):
        ...
        # Your Code here
        temp=1-Y*self.hypothesis(X,theta)
        temp[temp<0]=0;
        t=theta
        t[-1]=0
    ...
    def derivative_cost_function(self, X, Y, theta):
        ...
        # Your Code here
        #check_cond = Y * self.hypothesis(X,theta)
        #print "Check 1: ",np.shape(check_cond)
        #indx = np.argwhere(check_cond[:,0]>=0)
        #print "Check 2: ",indx
        #deriv = -Y*X
        #print "Check 3: ",np.shape(deriv)
        #deriv[indx[:,0]] = 0
        #deriv = np.mean(deriv,axis=0).reshape(np.shape(X)[1],1)
        #theta_copy = np.copy(theta)
        #theta_copy[-1] = 0
        #print "Check 4: ",np.shape(deriv)
        #print "Check 5: ",np.shape(deriv + self.lembda*theta_copy)
        #return deriv + self.lembda*theta_copy
        temp=-Y*X
