...
class Optimizer:
    ...
    def gradient_descent(self, X, Y, cost_function, derivative_cost_function):
        ...
        thetas_new = [] 
        for itr in range(self.maxniter):
            thetas_new = derivative_cost_function(X,Y,thetas)
            thetas_new = thetas_new.reshape(X.shape[1],1)
            #if itr%20 == 0:
                #ypred = hypothesis_function(X,thetas_new)
                #plt.plot(X[:,0],ypred,c='r')
                #plt.scatter(X[:,0],Y,c='g')
            if np.linalg.norm(thetas-thetas_new) < eps:
                break
            thetas = thetas - self.alpha * thetas_new
        return thetas
hypothesis_function = lambda X,theta: 1/(1+np.exp(-1 * np.dot(X,theta)))